...
void set_socket_timeouts(int fd, int timeout_ms) {
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_SNDTIMEO, &timeout, sizeof(timeout)) == -1) {
        throw std::runtime_error(system_error_message("setsockopt(SO_SNDTIMEO) failed"));
    }
}
...
void TcpSocket::connect_to(
    ...
    if (gai_result != 0) {
        throw std::runtime_error(
            "getaddrinfo failed for host '" + host + "': " + gai_strerror(gai_result)
        );
    }
    ...
    throw std::runtime_error(
        "Unable to connect to " + host + ":" + std::to_string(port) +
...
std::string TcpSocket::receive_until(
