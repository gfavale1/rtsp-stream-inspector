...
int main(int argc, char **argv) {
  ...
  auto probe_cmd =
      app.add_subcommand("probe", "Send RTSP OPTIONS and DESCRIBE requests");
  ...
  try {
    ...
    if (*probe_cmd) {
...

      {
        ...
        std::string raw_response = read_full_rtsp_response(socket);
        auto response = rtsi::RtspResponse::parse(raw_response);

        std::cout << "\n--- RTSP DESCRIBE RESPONSE ---\n";
        print_response_summary(response);

        if (response.status_code() == 401 && parsed_url.has_credentials()) {
          ...
          raw_response = read_full_rtsp_response(socket);
          response = rtsi::RtspResponse::parse(raw_response);
        ...
        const auto sdp = rtsi::SdpParser::parse(response.body());
        ...
        if (video_track.has_value()) {
          ...
          std::cout << "Video SETUP URI: "
                    << rtsi::build_control_uri(request_uri,
                                               video_track->control)
                    << '\n';
        }
